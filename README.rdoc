...
- It uses migrations, models, controllers, views, form helpers, partials, flash errors, active record usage, 
boostrap, seeds, routes, db config.
- Stores search history in mysql.
- Rails 4.1.0
- Relies on goolgle books api to perform search.
- Search uses Rest ORM (HER) for rails to perform rest request to google's api.
